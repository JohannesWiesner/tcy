...
With a python installation on their machines, users can run the  `tcy.py` script in the console (`tcy.py` will expect to find a `packages.tsv` file in the current working directory):
...
This repository contains a subfolder called `environments` . Users can download this repository and use the  `.yml` files within that folder to create a conda environment. The files contain solved solutions of the package specifications found in `packages.tsv` for both Windows and Ubuntu. This `.tsv` file contains all packages that the Complex Systems in Psychiatry Lab needs.
...
Whenever you find a new package that you find interesting and/or have to install for your project, you can open an issue in this repository or even better, fork this repo, add your package to the `packages.tsv` file and send a pull request!
